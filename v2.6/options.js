...
async function initialize() {
  ...
  setVisibilityButton(elements.topNavigationToggle, settings.showTopNavigation, "topNavigation");
  setVisibilityButton(elements.inputCardToggle, settings.showInputCard, "inputCard");
...
function bindEvents() {
  ...
  elements.topNavigationToggle.addEventListener("click", () => {
    toggleVisibilityButton(elements.topNavigationToggle, "topNavigation");
  });
  elements.inputCardToggle.addEventListener("click", () => {
    toggleVisibilityButton(elements.inputCardToggle, "inputCard");
  });
}
...
function fillApiCard(card, api, index) {
  ...
  setVisibilityButton(card.querySelector(".toggle-api-card"), api.showCard, "apiCard");
...
function bindApiCard(card, index) {
  card.querySelector(".toggle-api-card").addEventListener("click", (event) => {
    toggleVisibilityButton(event.currentTarget, "apiCard");
  });
...
function syncStateFromDom() {
  settings = normalizeSettings({
    ...
    showTopNavigation: isVisibilityEnabled(elements.topNavigationToggle),
    showInputCard: isVisibilityEnabled(elements.inputCardToggle),
...
function readApiCard(card) {
  ...
  api.showCard = isVisibilityEnabled(card.querySelector(".toggle-api-card"));
  return api;
}
...
function updateInterfaceLanguage() {
  ...
  for (const card of elements.apiList.querySelectorAll(".api-card")) {
    ...
    setVisibilityButton(
      card.querySelector(".toggle-api-card"),
      isVisibilityEnabled(card.querySelector(".toggle-api-card")),
      "apiCard"
    );
  }
}

function toggleVisibilityButton(button, control) {
  setVisibilityButton(button, !isVisibilityEnabled(button), control);
}

function setVisibilityButton(button, visible, control) {
  button.setAttribute("aria-pressed", String(visible));
  button.querySelector(".toggle-status").textContent = t(visible ? "shown" : "hidden");
  const action = t(`${visible ? "hide" : "show"}${capitalize(control)}`);
  button.title = action;
  button.setAttribute("aria-label", action);
}

function isVisibilityEnabled(button) {
