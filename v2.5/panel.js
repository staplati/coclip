...
function renderResponseCards() {
  ...
  commitResponseEntries(settings.apis.map((api, index) => createResponseEntry(api, index)));
}
...
function reconcileResponseCards() {
  ...
  const nextEntries = settings.apis.map((api, index) => {
...
function updateResponseEmptyState() {
  elements.emptyState.hidden = settings.apis.length > 0;
...
function renderSectionNav() {
  ...
  responseEntries.forEach((entry, index) => {
    const button = createNavButton(entry.api.displayName || entry.api.modelId || t("apiFallback", { number: index + 1 }));
    button.dataset.apiIndex = String(index);
...
function renderClipboardPreview() {
  const visible = Boolean(clipboard.text || clipboard.images.length);
